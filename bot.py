...
class Holmes:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}   Holmes AI{Fore.BLUE + Style.BRIGHT} Daily Checkin BOT
            """
            f"""
    ...
    async def get_message(self, address: str, proxy_url=None, retries=5):
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}Status  :{Style.RESET_ALL}"
                    f"{Fore.RED+Style.BRIGHT} Fetch Message Failed {Style.RESET_ALL}"
                    f"{Fore.MAGENTA+Style.BRIGHT}-{Style.RESET_ALL}"
                    f"{Fore.YELLOW+Style.BRIGHT} {str(e)} {Style.RESET_ALL}"
                )
            ...
            if user:
                if user.get("RetCode") == 0:
                    ...
                    self.log(
                        ...
                        f"{Fore.WHITE+Style.BRIGHT} {points} Clue {Style.RESET_ALL}"
                    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.RED + Style.BRIGHT}[ EXIT ] Holmes AI Daily Check-in BOT{Style.RESET_ALL}                                       "                              
        )
